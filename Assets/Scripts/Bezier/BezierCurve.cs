...
public class BezierCurve : MonoBehaviour
{
    ...
    private void Start() 
    {
        ...
        {
            var tangentLineVertex1 = Vector3.Lerp(point3, point2, ratio);
            var tangentLineVertex2 = Vector3.Lerp(point2, point1, ratio);
        ...
        lineRenderer.SetPosition(pointList.Count + 3, transform.GetChild(0).position);
        lineRenderer.SetPosition(pointList.Count + 2, point1);
        lineRenderer.SetPosition(1, point3);
        lineRenderer.SetPosition(0, transform.GetChild(2).position);
    ...
    private void Update() 
    {
        ...
        lineRenderer.SetPosition(lineRenderer.positionCount - 1, transform.GetChild(0).position);

        if (lineRenderer.positionCount >= 2 && transform.GetChild(0).position == lineRenderer.GetPosition(lineRenderer.positionCount - 2))
        {
    ...
    private void OnDrawGizmos() 
    {
        ...
        Gizmos.DrawLine(transform.GetChild(1).position, transform.GetChild(2).position);
    }
}
